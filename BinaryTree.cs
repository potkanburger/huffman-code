...
namespace Huffman
{
    class BinaryTree
    {
        ...
        private Dictionary<char, Tuple<byte, int>> recursiveHuffman(Dictionary<char, Tuple<byte, int>> dicoHuffman, List<Nodes> nodeList, byte b, int len)
        {
            Dictionary<char, Tuple<byte, int>> resHuf = dicoHuffman;
            if (nodeList.Count > 1)
            {
